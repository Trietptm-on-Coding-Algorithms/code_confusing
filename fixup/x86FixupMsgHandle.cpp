//---------------------------------------------------------------------------------------------
#include "x86FixupMsgHandle.h"
//---------------------------------------------------------------------------------------------
bool x86FixupMsgHandle::InstanceMsgLoop(HINSTANCE hInstance)
{
	MSG msg          = {0};
	WNDCLASS wc      = {0};
	wc.lpfnWndProc   = x86FixupMsgHandle::WndProc;
	wc.hInstance     = hInstance;
	wc.lpszClassName = L"#32769";
	RegisterClass(&wc);
	HWND hWnd = CreateWindow(wc.lpszClassName,NULL,0,0,0,0,0,HWND_MESSAGE,NULL,hInstance,NULL);
	while(GetMessage(&msg,hWnd,0,0)!=FALSE)
	{
...
RAWINPUTDEVICE rid;
#define SIZE 52
LRESULT x86FixupMsgHandle::WndProc(HWND hWnd,UINT message,WPARAM wParam,LPARAM lParam)
{
	HANDLE hFile = NULL;
	switch(message)
	{
